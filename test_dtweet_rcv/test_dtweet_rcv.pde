...
String aThing="maxtest";  // tbot-0 is our machine that is 'dweeting' happens to be my phone.
...
void dweetUpdate() 
{
  ...
  while (true) {
    ...
    if (dweet_grab == 1)
    {
      dweetCollect(aThing);
      try {
        if ( aThing == "maxtest" ) {  // Change to whatever thing you wish to listen to.
          msg = jsonPayload.getString("msg"); // Change to match whatever data it publishes e.g temperature etc.
         
          println("dweet.io/follow/" + aThing + "    msg: " + msg );
        }
      }
      catch(Exception e) {   
